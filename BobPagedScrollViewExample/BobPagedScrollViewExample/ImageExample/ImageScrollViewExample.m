...
@implementation ImageScrollViewExample
...
-(BobPage *) bobPagedScrollView:(BobPagedScrollView *)bobPagedScrollView pageForIndex:(NSUInteger)index {
    BobImagePage *page = (BobImagePage *)[bobPagedScrollView dequeueReusablePageWithIdentifier:@"test"];
    if (!page) {
        page = [[[BobImagePage alloc] initWithFrame:CGRectMake(0.0f,0.0f,self.view.frame.size.width, self.view.frame.size.height) andReuseIdentifier:@"test"] autorelease];
    }
    
    page.imageView.image = [UIImage imageNamed:[images objectAtIndex:index]];
