...
@interface BobCenteringImageScrollView ()
...
-(id) initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		imageView = [[TapDetectingImageView alloc] initWithFrame:CGRectZero];
...
-(void) updateFrame:(CGRect)theFrame {
    ...
    if (thumbnail) {
        ...
        self.zoomScale = 1.0f;
        imageView.frame = CGRectMake(0.0f, 0.0f, imageView.image.size.width, imageView.image.size.height);
        [self setImageToBoundsSize:theFrame.size andImageSize:imageView.image.size];
    } else {
        self.zoomScale = 1.0f;
        imageView.frame = CGRectMake(0.0f, 0.0f, imageView.image.size.width, imageView.image.size.height);
        [self setScrollViewZoomScalesForBounds:theFrame.size andImageSize:imageView.image.size];
    }
